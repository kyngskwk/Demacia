  ...
  <div style="heigth:100%; font-family:Tmon">
    <div class="d-flex row" style>
      ...
      <div
        ...
        style="padding-right:0;padding-left:0; margin-top:5%; max-width:400px; "
      >
        <div id="pad1" class="shadow1">
          <div style=" background-color:black; height:200px; width:100%; ">
            <div class="container" style="padding-top:20px;">
              ...
              <div v-b-modal.modalProfileImage style="cursor:pointer;">
                ...
                <!-- 이미지 변경 모달 -->
                <b-modal id="modalProfileImage" size="sm" title="프로필 사진 변경" centered hide-footer>
                  <b-list-group>
                    <b-list-group-item @click="imgUp" href="#" variant="primary">내 기기에서 파일 탐색</b-list-group-item>
                    <b-list-group-item @click="imgDel" href="#" variant="primary">프로필 사진 삭제</b-list-group-item>
                    <b-list-group-item
                      href="#"
                      variant="secondary-outline"
                      v-b-modal.imageRule
                    >사진 업로드 이용규칙</b-list-group-item>
                    <b-modal id="imageRule" title="프로필 사진 업로드 이용규칙" ok-only centered>
                      프로필 사진은 회원 여러분의 개성을 드러낼 수 있는 서비스입니다.
                      <br />다만 원활한 서비스 제공에 문제를 일으키는 프로필 사진은 규제될 수 있음을 알려드리오니, 업로드 시 유의하여 주시기 바랍니다.
                      <br />[규제 대상]
                      <ol>
                        <li>음란, 선정성 사진</li>
                        ...
                        <li>명예훼손, 저작권 침해 등 타인의 권리를 침해하는 이미지</li>
                        <li>그 외 법령 및 약관에 위배되는 사진 및 이미지</li>
                      </ol>규제 대상으로 확인된 이미지는 발견 즉시 무통보 삭제되며, 정도와 빈도에 따라 서비스 이용이 제한 될 수 있습니다.
                      <br />이의 사항 발생 시 관리자에게 문의바랍니다.
                      <br />
                      <br />건전하고 멋진 프로필 사진 사용으로 여러분의 개성을 뽐내주세요. 감사합니다.
                    </b-modal>
                  </b-list-group>
                </b-modal>
                <input
          ...
          <div style=" background-color:white; height:170px;">
            <b-row style=" padding-left:30%;">
              <b-col cols="2" class="ml-1" style="font-size:20px;color:darkgray;">{{user.userEmail}}</b-col>
            </b-row>
              ...
              <b-col>
                ...
                <b-input-group v-show="!nicknameCheck">
                  ...
                  <b-input-group-append>
                    <b-button variant="light" @click="nicknameUpdate">수정</b-button>
                  </b-input-group-append>
            ...
            <b-row>
              <b-col>
                ...
                <b-modal ref="passwordChkModal" centered title="비밀번호 재입력">
                  <b-input-group>
                    ...
                    />
                    <b-form-invalid-feedback id="passwordChkFeedback">{{passwordChkMsg}}</b-form-invalid-feedback>
                  </b-input-group>
                  <template v-slot:modal-footer>
                    <b-button size="sm" variant="light" @click="passwordCheck">확인</b-button>
                  </template>
                </b-modal>
                <!-- 비밀번호 변경 모달 -->
                <b-modal ref="passwordUpdateModal" centered title="비밀번호 변경">
                  <div class="form-group">
                    ...
                    />
                    <b-form-invalid-feedback id="pwFeedback">{{passwordMsg}}</b-form-invalid-feedback>
                  </div>
                  <div class="form-group">
                    <b-form-input
                      type="password"
                    ...
                    />
                    <b-form-invalid-feedback id="pwconFeedback">{{passwordConfirmMsg}}</b-form-invalid-feedback>
                  </div>
                  <template v-slot:modal-footer>
                    <b-button size="sm" variant="light" @click="passwordUpdate">비밀번호 변경</b-button>
                  </template>
                </b-modal>
                <!-- 마일리지랭킹 조회 -->
              ...
              <b-col>
                ...
                <b-dropdown variant="light" text="더 보기" class="m-2">
                  <b-dropdown-item @click="(nicknameCheck = !nicknameCheck)">닉네임 수정</b-dropdown-item>
                  <b-dropdown-item @click="showPasswordChkModal">비밀번호 변경</b-dropdown-item>
      ...
      >
        <div style="height:120px;">
          <div
            ...
            style="width:300px; height:100px;background-color:white; margin:auto; top:50%; font-family:digital"
          >
            <p
              style="font-size:3vh;margin-top:10%; color:red; font-weight:bold; padding-right:4px;"
            >{{user.userMileage}}</p>
    ...
    <div style="background-color:rgb(222, 226, 228); margin:5%">
      ...
      <div class="row d-flex justify-content-center">
        <div class="shadow1 col-10 col-xl-5 m-4" style="background-color:white;">
          <!-- 솔 랭 -->
          <b-row>
            <b-col>
              <b-card no-body style="border:none;">
                <b-row no-gutters>
                  <b-col cols="6" md="4">
                    <b-card-img :src="solRankImg" width="100%" alt="게임랭크" class="rounded-0"></b-card-img>
                  </b-col>
                  <b-col cols="6" md="8">
                    <b-card-body>
                      <b-card-text>
                        <b-row cols="1" style="margin-top:10%;">
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">SOLOMON RANK</p>
                          </b-col>
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">{{solRank}}</p>
                          </b-col>
                        </b-row>
                      </b-card-text>
                    </b-card-body>
          ...
          <b-row>
            <b-col>
              <b-card no-body class="m-2" style="border:none;">
                <b-row no-gutters>
                  <b-col cols="6" md="4">
                    <b-card-img :src="gameRankImg" width="100%" alt="게임랭크" class="rounded-0"></b-card-img>
                  </b-col>
                  <b-col cols="6" md="8">
                    <b-card-body>
                      <b-card-text>
                        <b-row cols="1" align-v="stretch" v-show="user.summonerName">
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">GAME RANK</p>
                          </b-col>
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">{{gameRank}}</p>
                          </b-col>
                        ...
                        <b-row align-v="center" cols="1" v-show="!user.summonerName">
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">GAME RANK</p>
                          </b-col>
                          <b-col>
                            <p id="font1" style="font-size:3vh;font-weight:bold;">소환사 인증이 필요합니다!</p>
                          </b-col>
                          <b-col>
                            <!-- 소환사 인증 폼 -->
                            <b-input-group>
                              ...
                              />
                              <b-input-group-append>
                                <b-button variant="light" @click="summonerAuth">인증</b-button>
                              </b-input-group-append>
                              <b-form-invalid-feedback id="summonerFeedback">{{summonerMsg}}</b-form-invalid-feedback>
                            </b-input-group>
                          </b-col>
                        </b-row>
                      </b-card-text>
          ...
          <ul class="solomonul">
            <li v-for="(item, $index) in listsol" :key="$index">
              <div class="solomon">
                ...
                <div class="point1" v-if="item.voteResult == 'Win'">+{{item.point}}SOL</div>
                <div class="point2" v-else-if="item.voteResult == 'Proceeding'">{{item.point}}SOL</div>
