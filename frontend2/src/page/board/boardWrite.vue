    ...
    <form :action="action" @submit.prevent="onSubmit">
      ...
      <v-layout v-bind="{ [inline ? 'row' : 'column']: true }" v-if="!groupBy">
        <v-field
          ...
          v-model="model[name]"
        />
        <v-alert class="py-2" error v-model="hasError">
          <div v-for="error in errors" :key="error">
            {{ error.message }}
          </div>
        </v-alert>
        <slot />
        <v-flex class="pt-2 actions" xs12>
          <slot name="buttons">
            <v-btn class="ma-0" primary dark type="submit">
              {{ $t(submitButtonText) }}
              <v-icon right, dark> {{ submitButtonIcon }} </v-icon>
            </v-btn>
          </slot>
        </v-flex>
      </v-layout>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      const valid = global.validator.make(
        this.model,
        this.rules,
        this.messages
      );
